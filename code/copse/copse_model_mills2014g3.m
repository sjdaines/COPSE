classdef copse_model_mills2014g3 < handle
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%% COPSE for MATLAB
    ...
    methods
        ...
        function   [dSdt, D] = timederiv(obj, tmodel, S)  
            % Time derivative and diagnostic variables (COPSE Mills (2014) G3 equations)
            %
            % Args:
            ...
            D = copse_landbiota_mills2014g3( obj.pars, tmodel, S, D, D.TEMP );
            ...
            D = copse_landsurfaceareas_mills2014g3( obj.pars, tmodel, S, D );
            ...
            D = copse_weathering_rates_mills2014g3( obj.pars, tmodel, S, D );
            ...
            switch(obj.pars.f_fepb)
                ...
                case 'gcubed' %%%% remove copse_crash as in G3 model
                    D.fepb = (obj.pars.k6_fepb/obj.pars.k1_oxfrac)*(1-D.ANOX) ;
                otherwise
            ...
            switch obj.pars.f_bug_g32014_Srconc
                 case 'No'
                    %%%% what it should have been without bugs                   
                    Sr_ocean_relative = ( S.Sr_ocean / obj.pars.Sr_ocean0 ) ; %%% assume dependence on Sr conc in ocean
                case 'Yes'
                    %%%% outputs to sediments INCLUDING BUG FROM G3 PAPER: moldelta_Sr_ocean used instead of Sr_ocean, oops!
                    Sr_ocean_relative = ( S.moldelta_Sr_ocean / obj.pars.Sr_ocean0 );
                otherwise
                    error('unrecognized f_bug_g32014_Srconc %s', obj.pars.f_bug_g32014_Srconc);
            end
            ...
            whenstart_forwards = obj.Tstart + 4.5e9 ; %%% forwards time for start of this run
            D.sediment_sr_evol = sediment_rbsr*( 1 - exp(-lambda_Rb*( tforwards - whenstart_forwards ) ) ) ;  %%% term for added 8786sr fractionation
            D.delta_Sr_sed   = ( S.moldelta_Sr_sed / S.Sr_sed ) + D.sediment_sr_evol ; %%% increasing due to decay
            D.dmoldelta_Sr_sed_dt_Rb = 0;  
