function D = copse_landbiota_mills2014g3(pars, tmodel, S, D, TEMP )
% COPSE_LANDBIOTA COPSE OCT dynamic land vegetation model 

%%%% effect of temp on VEG
switch pars.f_bug_g32014_landbiotatemp
    case 'Yes'
        % effectively removed T limitation, visible as slightly high VEG 
        D.V_T = 1 - (( (TEMP - 298)/298 )^2) ;
    case 'No'
        D.V_T = 1 - (( (TEMP - paleo_const.k_CtoK-25)/25 )^2) ;
    otherwise
        error('unrecognized pars.f_bug_g32014_landbiotatemp %s',pars.f_bug_g32014_landbiotatemp);
end
% 
% 
