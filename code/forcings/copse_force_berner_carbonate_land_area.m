classdef copse_force_berner_carbonate_land_area < copse_force
    ...
    methods
        ...
        function load(obj)
            %%%% load data
            fprintf('loading carbonate land area from "%s"\n',obj.datafile);      
            eru = xlsread(obj.datafile);
            obj.data.tforwardsage           = eru(:,1);            
            obj.data.carbonate_land_area    = eru(:,2);
        end
        ...
        function D = force(obj, tforce_presentdayiszeroyr, D )                                 
            ...
            if    obj.extrapolate &&  (tforwardsage < obj.data.tforwardsage(1)) 
                D.CARB_AREA = obj.extrapval;
            elseif obj.extrapolate && (tforwardsage > obj.data.tforwardsage(end))
                D.CARB_AREA = obj.extrapval;
            else
                D.CARB_AREA   = interp1(obj.data.tforwardsage,obj.data.carbonate_land_area,tforwardsage) ;  
            end
        end
